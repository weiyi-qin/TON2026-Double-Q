...
OUTPUT_FIGURE = SCRIPT_DIR / "fixed0430.eps"
...
def load_results():
    ...
    for label, file_name, color, linestyle in RESULT_SPECS:
        ...
        with np.load(result_path) as data:
            ...
            current_seed = int(data["seed"].item())
    ...
    return loaded_items, total_steps, seed


def style_axis(ax, total_steps, font_family, tick_fontsize, legend_fontsize):
...
def draw(show=True):
    ...
    plot_items, total_steps, seed = load_results()
    ...
    style_axis(ax1, total_steps, font_family, tick_fontsize, legend_fontsize)
    ax1.yaxis.set_major_formatter(ScalarFormatter(useMathText=True))
    ax1.ticklabel_format(style="sci", axis="y", scilimits=(0, 0))
    ax1.yaxis.offsetText.set_fontsize(offset_fontsize)
    ax1.yaxis.offsetText.set_fontname(font_family)
    ...
    style_axis(ax2, total_steps, font_family, tick_fontsize, legend_fontsize)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Draw fixed constraint experiment results.")
    parser.add_argument(
        "--no-show",
        action="store_true",

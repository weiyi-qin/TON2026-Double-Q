...
def soft_project_to_constraint(vector, bound, max_iter=80):
    bound = torch.as_tensor(bound, dtype=vector.dtype, device=vector.device)
...
def distance_and_gradient_to_constraint(vector, bound, scale=1.0):
    projection = soft_project_to_constraint(vector, bound)
...
def run_double_dq(raw_data):
    ...
    lam = torch.sqrt(torch.tensor(float(step), device=DEVICE))
    u_dq = 1.0
    v_min = 0.1 * epsilon * step
    eps_dq = 1 / step

    zq = torch.zeros(1, dtype=torch.float32, device=DEVICE)
    zq_hat = v_min * torch.ones(1, dtype=torch.float32, device=DEVICE)
    gradient_norms = []
    averaged_loss = []
    averaged_violation = []
    time_loss = 0.0
    ...
    for t in range(1, step):
        ...
        distance_prev, distance_grad_prev = distance_and_gradient_to_constraint(
            vector,
            constraint_upper[t - 1],
            scale=d_lip,
        )
        zq = zq + beta_dq * distance_prev
        psi_prime = zq + lam

        gradient = u_dq * (loss_grad + distance_grad_prev) + beta_dq * psi_prime * distance_grad_prev
        gradient_norm = torch.sum(gradient * gradient)
        gradient_norms.append(gradient_norm)
        alpha = np.sqrt(1) * radius / torch.sqrt(torch.stack(gradient_norms).sum() + 1e-8)

        candidate = vector - alpha * gradient
        if zq_hat.item() > v_min:
            candidate = candidate - 0.1 * alpha * zq_hat * distance_grad_prev
        new_vector = soft_project_to_constraint(candidate, constraint_upper[t - 1])

        assign_flat_parameters(model, new_vector)
        distance_new, _ = distance_and_gradient_to_constraint(
            flatten_parameters(model),
            constraint_upper[t],
            scale=d_lip,
        )
        zq_hat = torch.max(
            torch.tensor(v_min, dtype=torch.float32, device=DEVICE),
            (1 - eps_dq) * zq_hat + distance_new,
        )
...
def run_experiment():
    ...
    print(f"DoubleDQ algorithm completed in {elapsed:.2f} seconds. Results saved to {output_path}")

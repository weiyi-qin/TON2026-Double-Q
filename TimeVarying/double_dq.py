...
def double_dq_centralized(
    ...
    _, grad_d_prev = distance_and_gradient(x_prev, a_estimate, b_estimate, d_lip)

    for i in range(CONSTRAINT_DIM):
        d_i, _ = distance_and_gradient(
            x_prev,
            a_estimate[i : i + 1],
            b_estimate[i : i + 1],
            d_lip,
    ...
    for i in range(CONSTRAINT_DIM):
        _, grad_d_i = distance_and_gradient(
            x_prev,
            a_estimate[i : i + 1],
            b_estimate[i : i + 1],
            d_lip,
    ...
    for i in range(CONSTRAINT_DIM):
        ...
        constraints += [
            y_i >= X_LOWER,
            y_i <= X_UPPER,
            a_estimate[i : i + 1] @ y_i <= b_estimate[i : i + 1],
        ]
    ...
    for i in range(CONSTRAINT_DIM):
        d_i_new, _ = distance_and_gradient(
            x_new,
            a_current[i : i + 1],
            b_current[i : i + 1],
            d_lip,
